...
| `DATABASE_URL` | PostgreSQL 服务的连接信息。来源可以是自建 PostgreSQL、云数据库控制台，或服务器本机数据库配置。 | 需要持久化数据时必填 | `DATABASE_URL=postgresql://user:password@127.0.0.1:5432/ai_mock_coach?schema=public` |
...
如果暂时不需要 PostgreSQL、Redis 或外部 AI provider，可以删掉对应行。项目会使用代码里已有的本地兜底逻辑。
...
当前 CI/CD 工作流不会自动执行生产数据库迁移。如果你在 `PRODUCTION_ENV` 中配置了 `DATABASE_URL`，请先确保生产库 schema 已经创建好，再切真实流量。
