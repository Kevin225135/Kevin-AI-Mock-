...
- AI provider 统一从 `src/lib/ai/scorer.ts` 编排；Ark 使用 Responses API，其他 OpenAI-compatible 服务使用 `/chat/completions`。
...
维度分为 1 到 5，总分为 0 到 100。LLM provider 必须返回 JSON；本地 scorer 用同一 schema 兜底。Ark 配置使用 `AI_PROVIDER="ark"`、`ARK_API_KEY`、`ARK_MODEL` 和 `ARK_API_BASE_URL`。
