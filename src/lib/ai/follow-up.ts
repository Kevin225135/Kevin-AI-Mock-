...
import type { FollowUpDecision } from "./follow-up-decision";
export { decideFollowUp } from "./follow-up-decision";

export async function createFollowUpQuestion(input: {
  module: InterviewModule; targetRole: string; difficulty: Difficulty;
  originalPrompt: string; answer: string; round: number; decision: Exclude<FollowUpDecision, "CLOSE">;
}) {
  const prompt = input.decision === "CLARIFY"
    ? `请重新聚焦这道题，并补充必要背景：你的具体角色、目标和需要解决的核心问题是什么？`
    : buildDeepenPrompt(input.answer);
  const row = await prisma.questionBank.create({ data: {
    externalId: `followup-${randomUUID()}`, module: input.module, targetRole: input.targetRole,
    difficulty: input.difficulty, prompt,
...
function buildDeepenPrompt(answer: string) {
  if (/\d+[%x]?|\d+\s*(人|天|周|月|元|万)/i.test(answer) && !/(因为|依据|验证|because|validated)/i.test(answer))
