...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "请填写问题描述。" }, { status: 400 });
    const store = await getDataStore();
    await store.trackEvent({ name: analyticsEvents.badcaseReport, sessionId, userId: actor.id, payload: { ...parsed.data, status: "OPEN", severity: parsed.data.type === "HALLUCINATION" ? "P0" : "P1" } });
    return NextResponse.json({ ok: true });
  } catch (error) { return jsonError(error, "提交问题失败。"); }
}
